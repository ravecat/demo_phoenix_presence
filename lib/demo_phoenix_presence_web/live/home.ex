defmodule DemoPhoenixPresenceWeb.Live.Home do
  use DemoPhoenixPresenceWeb, :live_view

  alias DemoPhoenixPresenceWeb.Presence

  def render(assigns) do
    ~H"""
    <Layouts.flash_group flash={@flash} />
    <div class="left-[40rem] fixed inset-y-0 right-0 z-0 hidden lg:block xl:left-[50rem]">
      <div :if={@current_scope} class="absolute bottom-4 right-4 w-64 gap-2 flex flex-col">
        <div id="messages" role="log" aria-live="polite"></div>
        <input
          id="chat-input"
          type="text"
          class="w-full rounded-lg border border-base-300 bg-base-200 px-4 py-2"
    ...
    socket =
      if connected?(socket) do
        case socket.assigns do
          ...
          _ ->
            # If current_scope or user are not set, just return the socket
            socket
        end
      else
        socket
